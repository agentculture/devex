...
# `devex pr` — PR lifecycle for agents
...
no committed removal date), so devex never posts `/improve`.

- `devex pr open` posts `/agentic_review` **automatically** for a freshly created,
  ...
  thread).
- `devex pr review [<PR>]` posts the same trigger on demand — use it to re-trigger
...
2. `[pr] sonar_project_key` in `.devex/config.toml`.
3. `<owner>_<repo>` (SonarCloud GitHub-import default).

When the project isn't on SonarCloud the API 404s and devex silently
