"""Render the trailing 'Next step:' footer for `devex overview`.

Thin shim over the command-agnostic :mod:`devex.core.footer` renderer. The
only ``overview``-specific knowledge here is *where* the per-backend hints
live — the ``commands/overview/assets/backends`` package — which this module
...
_BACKENDS_PKG = "devex.commands.overview.assets.backends"
