"""Render the trailing 'Next step:' footer for `devex explain`.

Thin shim over the command-agnostic :mod:`devex.core.footer` renderer. The
only ``explain``-specific knowledge here is *where* the per-backend hints
live — the ``commands/explain/assets/backends`` package — which this module
...
_BACKENDS_PKG = "devex.commands.explain.assets.backends"
