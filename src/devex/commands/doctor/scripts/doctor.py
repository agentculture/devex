"""`devex doctor` — read-only health check.
...
# (sonar python:S1192).
_NAME_AGEX_VERSION = "devex version"
_NAME_PYTHON = "Python"
_NAME_PACKAGE_RESOURCES = "Package resources"
_NAME_AGEX_DIR = "`.devex/` directory"
_NAME_CONFIG_TOML = "`.devex/config.toml`"
_NAME_GITIGNORE = "`.devex/.gitignore`"
_NAME_DATA_DIR = "`.devex/data/`"
...
def _doctor_assets() -> Traversable:
    return files("devex.commands").joinpath("doctor", "assets")
...
def _check_version() -> CheckResult:
    if not __version__:
        return CheckResult(
            _NAME_AGEX_VERSION,
            "fail",
            "Could not resolve `devex.__version__`. Reinstall with "
            "`uv pip install -e .[dev]`, `pipx install devex-cli`, "
            "`pipx install agent-devex`, or `pipx install agex-cli`.",
        )
...
def _check_resources() -> CheckResult:
    ...
    required = [
        ("explain", "assets", "topics", "devex.md"),
...
def _check_agex_dir() -> CheckResult:
    ...
    if not root.exists():
        return CheckResult(
            ...
            "(e.g. `devex overview --agent claude-code`) to bootstrap.",
        )
    if not root.is_dir():
        return CheckResult(
            _NAME_AGEX_DIR,
...
def _check_config_toml() -> CheckResult:
    ...
    if not path.exists():
        return CheckResult(
            _NAME_CONFIG_TOML,
            "info",
            "not present (expected when `.devex/` is uninitialized).",
        )
    ...
    try:
        from devex.core import config as config_module
...
def _check_gitignore() -> CheckResult:
    ...
    gi = root / ".gitignore"
    if not root.exists():
        return CheckResult(_NAME_GITIGNORE, "info", "skipped (no `.devex/`).")
    if not gi.exists():
        return CheckResult(
            _NAME_GITIGNORE,
            "warn",
            "missing — `data/` may end up tracked. Re-run any devex command to restore.",
        )
    try:
...
def _check_data_dir() -> CheckResult:
    if not state_dir().exists():
        return CheckResult(_NAME_DATA_DIR, "info", "skipped (no `.devex/`).")
    d = data_dir()
    if not d.exists():
        return CheckResult(
            _NAME_DATA_DIR,
            "warn",
            f"`{d}` is missing — re-run any devex command to recreate it.",
        )
    if not d.is_dir():
...
_OPERATOR_CHECKLIST = [
    "Confirm `.devex/config.toml` is committed and `.devex/data/` is gitignored.",
    "Confirm your shell tool can invoke `devex --version` and `devex doctor`.",
    "If you installed hooks via `devex gamify`, confirm the backend hook file "
    "still contains the `agex:` fragment IDs recorded in `.devex/config.toml`.",
]
...
def run(role: str | None = None) -> tuple[str, int, str]:
    ...
    try:
        ...
        return (
            ...
            1,
            error_prefix(
                f"could not read `doctor/assets/report.md.j2`: {exc}. " "Reinstall the package."
            ),
        )
    out = render_string(
        template_text,
